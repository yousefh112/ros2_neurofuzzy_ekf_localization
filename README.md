...
- **Geofence source:** `/odom` (not `/odometry/global`, GPS bias 0.25 to 0.99 m causes premature firing on the global frame)
...
7 doors × 3 passes = 21 guarded waypoints. Stuck recovery: 8 s timeout, then 1.5 s backup + 1.5 s rotate-to-face.
...
All 7 door guards cleared on first approach. The south-exit guard (`s_exit`) showed an expected 2 to 3 cycle oscillation due to ANN lateral noise near the alignment threshold, resolving within 2 seconds without triggering stuck recovery.
